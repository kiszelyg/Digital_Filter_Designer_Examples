...
#include "MyFilter_iiri64cu.hpp"  // STEP 0: Include the generated header file of the filter implementation
...
int main()
{
	CMyFilterIirI64Cu l_myFilter; // STEP 1: Make a filter instance
	...
	while (l_recentFreq_f64 < l_finalFreq_f64)
	{
		...
		for (unsigned long l_sampleCntr_ui64 = 0; l_sampleCntr_ui64 < (l_numOfSamples_ui64 + l_startAtSample_ui64); l_sampleCntr_ui64++)
		{
			...
			l_myFilter.doRwdFiltering(); // STEP 5: Do the rewards filtering, this function can be called parallely, calculations must be done before the next call of doFiltering
	...
	l_myFilter.destroyFilter(); // STEP 6: Clean up the memory at the end of the operation
	return 0;
}
