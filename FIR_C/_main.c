...
#include "MyFilter_firi32c.h" // STEP 0: Include the generated header file of the filter implementation
...
int main()
{
	struct CFirI32CData l_myFilter; // STEP 1: Make a filter instance
	if (RETURN_OK != CMyFilterFirI32C_initFilter(&l_myFilter)) // STEP 2: Initialise it
	{
	...
	{
		CFirI32C_resetFilter(&l_myFilter); // STEP 3: Reset the filter between the frequency steps
		...
		for (unsigned long l_sampleCntr_ui64 = 0; l_sampleCntr_ui64 < (l_numOfSamples_ui64 + l_startAtSample_ui64); l_sampleCntr_ui64++)
		{
			...
			long l_realInputVal = (long)(round(l_inputVal_f64)); // Input value must fit to the filter data type
			long l_realOutputVal = 0.0; // Input value must fit to the filter data type

			l_realOutputVal = CFirI32C_doFiltering(&l_myFilter, l_realInputVal); // STEP 4: Call the filter with the recent input value
	...
	CMyFilterFirI32C_destroyFilter(&l_myFilter); // STEP 5: Clean up the memory at the end of the operation
