...
#include "MyFilter_firi64c.h" // STEP 0: Include the generated header file of the filter implementation
...
int main()
{
	struct CFirI64CData l_myFilter; // STEP 1: Make a filter instance
	if (RETURN_OK != CMyFilterFirI64C_initFilter(&l_myFilter)) // STEP 2: Initialise it
	{
		printf("Implement your failure handling for this case!\n");
		return RETURN_ERROR;
	}
	CMyFilterFirI64C_initFilter(&l_myFilter); // STEP 2: Initialise it
	...
	{
		CFirI64C_resetFilter(&l_myFilter); // STEP 3: Reset the filter between the frequency steps
		...
		for (unsigned long l_sampleCntr_ui64 = 0; l_sampleCntr_ui64 < (l_numOfSamples_ui64 + l_startAtSample_ui64); l_sampleCntr_ui64++)
		{
			...
			l_realOutputVal = CFirI64C_doFiltering(&l_myFilter, l_realInputVal); // STEP 4: Call the filter with the recent input value
	...
	CMyFilterFirI64C_destroyFilter(&l_myFilter); // STEP 5: Clean up the memory at the end of the operation
