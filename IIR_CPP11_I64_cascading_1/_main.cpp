...
#include "HPF_iiri64cpp11.hpp"  // STEP 0: Include the generated header file of the filter implementation
#include "BSF_iiri64cpp11.hpp"  // STEP 0: Include the generated header file of the filter implementation
#include "LPF_iiri64cpp11.hpp"  // STEP 0: Include the generated header file of the filter implementation
...
int main()
{
	CHPFIirI64Cpp11 l_hpfFilter; // STEP 1: Make a filter instance
	CBSFIirI64Cpp11 l_bsfFilter; // STEP 1: Make a filter instance
	CLPFIirI64Cpp11 l_lpfFilter; // STEP 1: Make a filter instance
	...
	{
		l_hpfFilter.resetFilter(); // STEP 2: Reset the filter between the frequency steps
		l_bsfFilter.resetFilter(); // STEP 2: Reset the filter between the frequency steps
		l_lpfFilter.resetFilter(); // STEP 2: Reset the filter between the frequency steps
		...
		for (unsigned long l_sampleCntr_ui64 = 0; l_sampleCntr_ui64 < (l_numOfSamples_ui64 + l_startAtSample_ui64); l_sampleCntr_ui64++)
		{
			...
			l_realOutputVal = l_hpfFilter.doFiltering(l_realInputVal); // STEP 3: Call the filter with the recent input value
			l_realOutputVal = l_bsfFilter.doFiltering(l_realOutputVal);
			l_realOutputVal = l_lpfFilter.doFiltering(l_realOutputVal);

			l_hpfFilter.doRwdFiltering(); // STEP 4: Do the rewards filtering, this function can be called parallely, calculations must be done before the next call of doFiltering
			...
			l_lpfFilter.doRwdFiltering(); // STEP 4: Do the rewards filtering, this function can be called parallely, calculations must be done before the next call of doFiltering
